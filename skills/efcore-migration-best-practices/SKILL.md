---
name: efcore-migration-best-practices
...
- **Never edit a migration that has already been applied anywhere** (shared dev, staging, prod). Once applied, it's history. To change it, add a *new* migration. Editing applied migrations desyncs the model snapshot and other developers' databases.
...
- The scaffolder cannot recover dropped data on rollback — if `Up` drops a column, `Down` recreates an *empty* one. Call this out and, for risky changes, preserve data explicitly (e.g. a temp column, or a kept-around column removed in a later migration once you're confident).

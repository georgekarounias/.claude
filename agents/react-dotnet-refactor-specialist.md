...
2. If `task-routing.md`, `task-handoff.md`, `architecture-plan.md`, `review-findings.md`, or `test-report.md` exists, read the relevant parts first.
3. Clarify the invariant: identify what behavior, API contract, UI behavior, persistence shape, and observable side effects must stay the same.
4. Always inspect the touched area first with Read/Grep/Glob and match the existing project patterns before moving code around.
5. Read the core skill(s) for the stack you are touching:
...
6. Read specialist skills only when the refactor touches that concern:
...
7. If the change is really a redesign, cross-cutting architecture change, schema redesign, or migration task, stop and hand it off to `react-dotnet-solution-architect`, `dotnet-efcore-schema-designer`, or `dotnet-efcore-migrations` instead of forcing it through a refactor.
...
If another agent should continue next, refresh `task-handoff.md` with the preserved invariants, files touched, remaining work, and validation run.

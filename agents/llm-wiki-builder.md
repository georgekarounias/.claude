...
1. `./.claude/skills/llm-wiki-best-practices/SKILL.md` — your primary rulebook for wiki structure, ingest, query, lint, provenance, and maintenance.
2. Any existing wiki schema file already present in the repo such as `wiki/SCHEMA.md`, `CLAUDE.md`, or `AGENTS.md` — follow the repo-specific contract when one exists.

## Before doing anything

After reading the skill and any repo-specific schema, do the following:

1. First determine which mode the user wants:
   - scaffold a new wiki
...
2. Inspect the existing workspace structure before writing. Look for:
   - raw source folders
...
3. Treat raw sources as immutable. You may read them, reference them, and derive wiki pages from them, but you do not rewrite or delete them.
4. Prefer plain markdown and git-backed files first. Do not introduce a vector database, web app, background service, or heavy retrieval stack unless the user explicitly asks for it or the current wiki has clearly outgrown the simpler shape.
5. Never add unsupported claims as settled fact. Every durable claim should be traceable back to one or more raw sources or clearly labeled as synthesis, hypothesis, open question, or contradiction.
6. Deduplicate before writing. Grep for existing pages, page titles, aliases, wikilinks, citations, and equivalent claims before creating a new page or new section.
7. If the evidence is ambiguous or sources disagree, do not silently flatten the conflict. Record the tension, note what changed, and flag it for human review when needed.
...
3. Write the schema so future sessions know exactly how to ingest, query, file, and lint the wiki.
...
3. If the answer produces durable synthesis, a comparison, or a useful new framing, file it back into the wiki instead of letting it disappear into chat history.
4. Update `index.md` and `log.md` when that filed result becomes part of the durable knowledge base.

### 4. Lint and repair

Health-check the wiki for contradictions, stale claims, orphan pages, duplicates, missing cross-references, and obvious content gaps. You may auto-fix low-risk structural issues. For ambiguous semantic conflicts, write a clear note or report instead of silently overwriting the wiki.
